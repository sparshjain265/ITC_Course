...
class huffmanCoding:
	...
	def compress(self):
		...
		with open(self.path, 'r+') as file, open(output_path, 'wb') as output, open(meta_data, 'w') as m:
			temp = file.read()
			r = len(temp) % self.n
			while(r > 0):
				temp = temp + '\n'
			...
			freq = self.getFrequency(temp)
			sfreq = sum(freq.values())
			self.entropy = 0
			...
			for k, f in self.frequency.items():
				p = f/s
				self.jointEntropy -= p * math.log2(p)
				self.expectedLength += p * len(self.codes[k])
			self.jointExpectedLength = self.expectedLength
			self.expectedLength = self.expectedLength / self.n
			...
			json.dump(self.reverse_mapping, m)
			output.write(bytes(b))

		print("\nCompressed to " + output_path)
		print("Entropy: " + str(self.entropy))
		print("Expected Length: " + str(self.expectedLength))
		print("Joint Entropy: " + str(self.jointEntropy))
		print("Joint Expected Length: " + str(self.jointExpectedLength))
		# print("\nCodebook:-")
		# self.printCodes()
		return output_path
	...
	def decompress(self):
		...
		meta_data = filename + ".meta"
		with open(input_path, 'rb') as file, open(output_path, 'w') as output, open(meta_data, 'r') as m:
			...
			self.reverse_mapping = json.load(m)
